@extends('layouts.dashboard', [
    'title' => 'Buat Promo Bundling',
    'pageTitle' => 'Buat Promo Bundling',
    'breadcrumb' => '<li class="breadcrumb-item"><a href="'.route('dashboard').'">Home</a></li><li class="breadcrumb-item"><a href="'.route('bundle-promos.index').'">Promo Bundling</a></li><li class="breadcrumb-item">Buat Baru</li>',
])

@section('content')

<form action="{{ route('bundle-promos.store') }}" method="POST" id="bundle-form" class="mt-3">
    @csrf
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="bundle_code" class="form-control" value="{{ old('bundle_code', $autoCode) }}" required>
                        </div>
                        <div class="col-md-8">
                            <label class="form-label fw-semibold">Nama Paket Promo <span class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control" placeholder="Contoh: Promo Bundling Lampu + Fitting Broco" value="{{ old('name') }}" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">Keterangan / Syarat Ketentuan</label>
                            <textarea name="description" class="form-control" rows="2" placeholder="Catatan atau informasi tambahan promo">{{ old('description') }}</textarea>
                        </div>
                        <div class="col-md-4">
                                ...
                                <tr class="item-row" data-index="0">
                                    ...
                                    <td>
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="number" name="items[0][qty]" class="form-control text-center qty-input" min="1" value="1" required>
                                            <button type="button" class="btn btn-outline-secondary btn-qty-plus">+</button>
                                        </div>
                                    </td>
                                    <td>
                                        <input type="number" name="items[0][unit_price]" class="form-control form-control-sm text-end price-input" min="0" step="100" value="0" required>
                                    </td>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select name="discount_type" id="discount_type" class="form-select" required>
                            <option value="fixed" {{ old('discount_type', 'fixed') === 'fixed' ? 'selected' : '' }}>Potongan Nominal Tetap (Rp)</option>
                            <option value="percentage" {{ old('discount_type') === 'percentage' ? 'selected' : '' }}>Diskon Persentase (%)</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            <span class="input-group-text" id="discount_prefix">Rp</span>
                            <input type="number" name="discount_value" id="discount_value" class="form-control text-end fw-bold" min="0" step="1" value="{{ old('discount_value', 0) }}" required>
                            <span class="input-group-text d-none" id="discount_suffix">%</span>
                        </div>
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="feather icon-save me-1"></i> Simpan Paket Promo
                        </button>
