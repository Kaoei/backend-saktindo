...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { font-family: DejaVu Sans, sans-serif; font-size: 6.8px; line-height: 1.12; color: #1f2937; }
...
<body>
    ...
    <div class="doc">
        <table class="header">
            <tr>
                <td>
                    <div class="brand">Warehouse Delivery</div>
                    <h1>SURAT JALAN</h1>
                </td>
                <td class="doc-number">
                    <span class="pill">{{ ucfirst($deliveryNote->status) }}</span>
                    <strong>{{ $deliveryNote->delivery_note_number }}</strong>
                    {{ optional($deliveryNote->delivery_date)->format('d/m/Y') }}
                </td>
            </tr>
        </table>
        ...
        <table class="items">
            <thead>
                <tr><th style="width: 10%;" class="center">No</th><th>Barang</th><th style="width: 18%;" class="right">Qty</th><th style="width: 14%;">Unit</th></tr>
            </thead>
            <tbody>
                @foreach($deliveryNote->items as $item)
                    <tr>
                        ...
                        <td>
                            <div class="product-name">{{ $item->salesOrderItem?->product_name }}</div>
                            <div class="muted">{{ $item->salesOrderItem?->product_code }}</div>
                        </td>
